"""
LCT001-8: (19x16)-7.57cm-(19x16)-7.57cm-(19x16) array of U(2.35)O2 fuel rods with 2.032 cm pitch (SSC-2.35-000-(055-068))
...
mat3.add_element("Zn", 1.238700e-04)
...
surf1 = openmc.model.RectangularParallelepiped(-65.482, 65.482, -16.256, 16.256, -499.995, 499.995)
# Entire Problem (BCD)
surf2 = openmc.model.RectangularParallelepiped(-95.482, 95.482, -46.256, 46.256, -19.11, 106.44, boundary_type="vacuum")
surf3 = openmc.XPlane(surface_id=3, x0=-26.874)
surf4 = openmc.XPlane(surface_id=4, x0=-19.304)
surf5 = openmc.XPlane(surface_id=5, x0=19.304)
surf6 = openmc.XPlane(surface_id=6, x0=26.874)
# Al-5052 lower plug
surf11 = openmc.ZCylinder(surface_id=11, r=0.558)
# U(2.35)O2 fuel
surf12 = openmc.ZCylinder(surface_id=12, r=0.558)
# Al-1100 upper plug
surf13 = openmc.ZCylinder(surface_id=13, r=0.558)
# Al-6061 clad/outer
...
# Lattice 3: 19x16 array
lattice3 = openmc.RectLattice(lattice_id=3)
lattice3.lower_left = [-19.304, -16.256]
...
cell1.translation = (-46.178, 0.0, 0.0)
...
cell5.translation = (46.178, 0.0, 0.0)
...
source.space = openmc.stats.Box((-1.0, -2.016, 44.72), (1.0, 2.016, 46.72))
