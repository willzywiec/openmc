...
double alpha_dc_ifp {0.0};
double alpha_dc_ifp_std {0.0};
...
void calculate_kinetics_parameters()
{
  ...
  if (n <= 0) {
    ...
    // Calculate IFP-weighted Lambda_eff, Lambda_p, and alpha if enabled and
    // tally exists.
    // Tally scores layout:
    ...
    // Formulas:
    //   Lambda_eff = ifp-time-numerator / (ifp-denominator * k_eff)
    //   l_p = ifp-prompt-time-numerator / ifp-prompt-denominator
    //   Lambda_p = l_p / k_p
    //   alpha_dc = -beta_eff / l_p,  alpha = (k_p - 1) / l_p
    if (settings::calculate_alpha && simulation::kinetics_tally_index >= 0 &&
      ...
      if (ifp_denom > 0.0 && simulation::keff > 0.0) {
        ...
        simulation::lambda_eff_ifp =
          ifp_time_numer / (ifp_denom * simulation::keff);
      }
      ...
      if (ifp_prompt_denom > 0.0 && simulation::keff > 0.0) {
        ...
        // Lambda_p = l_p / k_p for reporting
        if (kp > 0.0) {
          simulation::lambda_p_ifp = lp / kp;
        }

        // Using the prompt lifetime form of the alpha eigenvalue equations:
        //   alpha_dc = -beta_eff / l_p
        //   alpha = (k_p - 1) / l_p
        // where k_p = k_eff * (1 - beta_eff) is the prompt multiplication
        // factor.
        //
        // Physical interpretation:
        //   alpha < 0: subcritical (prompt neutrons decaying)
        //   alpha = 0: prompt critical
        //   alpha > 0: prompt supercritical (prompt neutrons growing)
        if (lp > 0.0) {
          // alpha_dc = -beta_eff / l_p
          simulation::alpha_dc_ifp = -beta / lp;

          // alpha = (k_p - 1) / l_p
          simulation::alpha_ifp = (kp - 1.0) / lp;
          ...
          if (n > 1) {
            ...
            // Error propagation for Lambda_eff = time_numer / (denom * k_eff)
            {
              ...
              double var_L =
                dL_dnumer * dL_dnumer * ifp_time_numer_std *
                  ifp_time_numer_std +
            ...
            // Error propagation for l_p = prompt_time_numer / prompt_denom
            double lp_std;
            {
              double dlp_dnumer = 1.0 / ifp_prompt_denom;
              double dlp_ddenom =
                -ifp_prompt_time_numer / (ifp_prompt_denom * ifp_prompt_denom);

              double var_lp = dlp_dnumer * dlp_dnumer *
                                ifp_prompt_time_numer_std *
                                ifp_prompt_time_numer_std +
                              dlp_ddenom * dlp_ddenom * ifp_prompt_denom_std *
                                ifp_prompt_denom_std;
              lp_std = std::sqrt(var_lp);
              simulation::lifetime_p_ifp_std = lp_std;
            }

            // Error propagation for Lambda_p = l_p / k_p
            // where k_p = k_eff * (1 - beta_eff)
            {
              double dLp_dlp = 1.0 / kp;
              // d Lambda_p / d k_eff = -l_p * (1 - beta) / k_p^2
              double dLp_dkeff = -lp * (1.0 - beta) / (kp * kp);
              // d Lambda_p / d beta_eff = l_p * k_eff / k_p^2
              double dLp_dbeta = lp * k / (kp * kp);

              double var_Lp = dLp_dlp * dLp_dlp * lp_std * lp_std +
                              dLp_dkeff * dLp_dkeff * simulation::keff_std *
                                simulation::keff_std +
                              dLp_dbeta * dLp_dbeta * simulation::beta_eff_std *
                                simulation::beta_eff_std;
              simulation::lambda_p_ifp_std = std::sqrt(var_Lp);
            }

            // Error propagation for alpha_dc = -beta_eff / l_p
            // d alpha_dc / d beta = -1/l_p
            // d alpha_dc / d l_p = beta_eff/l_p^2
            {
              double dA_dbeta = -1.0 / lp;
              double dA_dlp = beta / (lp * lp);

              double var_alpha = dA_dbeta * dA_dbeta *
                                   simulation::beta_eff_std *
                                   simulation::beta_eff_std +
                                 dA_dlp * dA_dlp * lp_std * lp_std;
              simulation::alpha_dc_ifp_std = std::sqrt(var_alpha);
            }

            // Error propagation for alpha = (k_p - 1) / l_p
            // d alpha / d k_p = 1/l_p, then chain rule through k_p:
            //   d alpha / d k_eff = (1 - beta)/l_p
            //   d alpha / d beta_eff = -k_eff/l_p
            // d alpha / d l_p = -(k_p - 1)/l_p^2
            {
              double dA_dkeff = (1.0 - beta) / lp;
              double dA_dbeta = -k / lp;
              double dA_dlp = -(kp - 1.0) / (lp * lp);

              double var_alpha = dA_dkeff * dA_dkeff * simulation::keff_std *
                                   simulation::keff_std +
                                 dA_dbeta * dA_dbeta *
                                   simulation::beta_eff_std *
                                   simulation::beta_eff_std +
...
void write_eigenvalue_hdf5(hid_t group)
{
  ...
  if (settings::calculate_k_prompt) {
    ...
    if (settings::calculate_alpha && settings::ifp_on) {
      ...
      array<double, 2> alpha_dc_ifp_vals {
        simulation::alpha_dc_ifp, simulation::alpha_dc_ifp_std};
      write_dataset(group, "alpha_dc_ifp", alpha_dc_ifp_vals);
...
void read_eigenvalue_hdf5(hid_t group)
{
  ...
  if (settings::calculate_k_prompt && object_exists(group, "k_prompt")) {
    ...
    if (settings::calculate_alpha && settings::ifp_on) {
      ...
      if (object_exists(group, "alpha_dc_ifp")) {
        array<double, 2> alpha_dc_ifp_vals;
        read_dataset(group, "alpha_dc_ifp", alpha_dc_ifp_vals);
        simulation::alpha_dc_ifp = alpha_dc_ifp_vals[0];
        simulation::alpha_dc_ifp_std = alpha_dc_ifp_vals[1];
      }
...
void setup_kinetics_tallies()
{
  ...
  // IFP scores for Lambda_eff, Lambda_p, and alpha calculation
  // Formulas:
  //   beta_eff = (k_eff - k_prompt) / k_eff
  //   Lambda_eff = ifp-time-numerator / (ifp-denominator * k_eff)
  //   l_p = ifp-prompt-time-numerator / ifp-prompt-denominator
  //   Lambda_p = l_p / k_p
  //   alpha_dc = -beta_eff / l_p,  alpha = (k_p - 1) / l_p
